...
\section{Meccanismi di controllo e di rendicontazione}
	...
	\subsection{Controllo ritardi di attività}  Per mantenere il controllo sull'andamento generale si è scelto di adottare dei metodi grafici.
		\subsubsection{Dettaglio attività} Il sistema di ticketing adottato permette di visualizzare in modo dinamico il diagramma di  \textit{Gantt\ped{G}} delle attività. Viene visualizzato:
		\begin{itemize}
		...
		\end{center}
		\begin{figure}[h]
			\caption{Diagramma di \textit{Gantt\ped{G}} prodotto dal sistema di ticketing adottato.}\label{etichetta}
		\end{figure}
		\subsubsection{Fasi processi}
		Combinando i dati del sistema di ticketing in un \textit{grafico ad area in pila\ped{G}} per visualizzare il numero di ticket aperti in un particolare stato del ciclo di \textit{Deming\ped{G}}, è immediato visualizzare in quali stati si trovino le attività.
		...
		\end{itemize}
		In particolare le metriche \textit{Budget Variance\ped{G}} (\textit{BV\ped{G}}) e \textit{Schedule Variance\ped{G}} (\textit{SV\ped{G}}) permettono rispettivamente di:
		\begin{itemize}
