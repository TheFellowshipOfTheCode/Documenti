...
\item \textbf{BLE\ped{G} (Bluetooth Low Energy)}: tecnologia Bluetooth a consumo ridotto di energia e costi che riesce a mantenere un range di comunicazione molto simile a quello fornito dal Bluetooth classico;
\item \textbf{Beacon\ped{G} (iBeacon/Eddystone)}: dispositivi beacons\ped{G} Apple/Google per gestire la microlocalizzazione indoor;
\item \textbf{HTML5\ped{G}, CSS3\ped{G}, jQuery\ped{G}, JavaScript\ped{G}}: consigliati per l'implementazione dell'interfaccia mobile;
...
\item \textit{Beacon\ped{G}} e \textit{BLE\ped{G}}: è richiesta una conoscenza approfondita dei dispositivi \textit{beacons\ped{G}} e del protocollo \ped{BLE\ped{G}}.
\end{itemize}
