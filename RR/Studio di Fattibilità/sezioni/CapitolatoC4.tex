...
\begin{itemize}
	\item \textbf{Node.js\ped{G}}: è un \textit{framework\ped{G}} \textit{event-driven\ped{G}} per il motore \textit{Javascript\ped{G}} sviluppato da Google. Si tratta quindi di un \textit{framework\ped{G}} relativo all'utilizzo server-side di \textit{Javascript\ped{G}}; 
	\item \textbf{MongoDB\ped{G}}: è un DBMS non relazionale, orientato ai documenti, classificato come \textit{NoSQL\ped{G}};
	\item \textbf{Git\ped{G}}: è un sistema software di controllo di versione distribuito; 
	\item \textbf{React.js\ped{G}}: è una libreria scritta in \textit{Javascript\ped{G}} open-source sviluppata da Facebook che consente di creare interfacce utente complesse tramite la loro suddivisione in componenti; 
...
Inoltre la mera gestione di database di tipo \textit{NoSQL\ped{G}} non risulta essere di gradimento da parte del gruppo: non ha tecnologie rilevanti rispetto ad altri capitolati. 
