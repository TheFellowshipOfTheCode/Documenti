...
Ultimato lo \textit{\SdF} gli \textit{Analisti} dovranno redigere l'\textit{\AdR} 
...
e/o esterne. Questo compito spetta agli \textit{Analisti}. I requisiti dovranno 
...
I \Progs sono responsabili delle attività di progettazione. Essi sono tenuti ad avere:
\begin{itemize} 
\item
...
\textbf{DocumentsDB} consente di esportare l'elenco degli attori, delle fonti, del glossario, dei casi d'uso, dei requisiti e delle loro relazioni come codice \LaTeX\ped{G}.
...
L'editor scelto dal team per sviluppare i documenti in \LaTeX\ped{G} è \textit{TexMaker}\ped{G}.  
\textit{TexMaker}\ped{G} è un editor gratuito, moderno e multi-piattaforma per Linux\ped{G}, sistemi Mac \textit{OS}\ped{G} e Microsoft \textit{Windows}\ped{G} che integra molti strumenti utili per sviluppare documenti in \LaTeX\ped{G}.
\textit{TexMaker}\ped{G} include il supporto \textit{Unicode}\ped{G}, il controllo ortografico, il completamento automatico, il raggruppamento del codice e un visore incorporato pdf\ped{G} con il supporto \textit{SyncTex}\ped{G} e modalità di visualizzazione continua.
...
\item
Consente di effettuare unit test per \textit{JavaScript}\ped{G} mediante il \textit{framework}\ped{G} \textit{Karma}\ped{G};
\item
...
\subsubsection{HTML5}
\textit{HTML}\ped{G}\textit{5} è un linguaggio di \textit{markup}\ped{G} per la strutturazione delle pagine web, derivato dallo standard che definisce \textit{HTML}\ped{G}.
...
\end{itemize}
\textit{HTML5}\ped{G} si presenta come un linguaggio pronto ad essere plasmato secondo le più recenti necessità, sia dal lato della strutturazione del contenuto che da quello dello sviluppo di vere e proprie applicazioni.
...
Questo linguaggio istruisce un \textit{browser}\ped{G} su come il documento debba essere presentato all'utente, per esempio definendone la formattazione del testo, il posizionamento degli elementi rispetto a diversi \textit{media}\ped{G} e \tetxit{device}\ped{G} eccetera.
