...
La progettazione deve utilizzare le seguenti tipologie di diagrammi UML\pad{G}:
\begin{itemize} 
\item
...
\item
\textbf{Diagrammi dei package}\pad{G}: Raggruppamenti di classi in una unità di livello più alto
\item
...
Texmaker\ped{G} è un editor gratuito , moderno e multi-piattaforma per Linux, sistemi MacOSX\ped{G} e Windows\ped{G} che integra molti strumenti utili per sviluppare documenti con LaTeX\ped{G}.
Texmaker\ped{G} include il supporto unicode , il controllo ortografico , il completamento automatico , il raggruppamento del codice  e un visore incorporato pdf con il supporto synctex\ped{G} e modalità di visualizzazione continua .
Texmaker\ped{G} è uno strumento facile da usare e da configurare.


\subsubsection{WebStorm}
L'ambiente di sviluppo integrato (IDE) utilizzato è \textbf{WebStorm}\ped{G}. Sono stati testati anche altri IDE\ped{G}, ma nessuno si è dimostrato all'altezza di \textbf{WebStorm}\ped{G}. Esso presenta le seguenti funzionalità:
\begin{itemize}
\item
Autocompletamento del codice JavaScript\ped{G}, HTML\ped{G} 5 e CSS3;
\item
Autocompletamento per metodi, funzioni e framework\ped{G} esterni, utili per il progetto\ped{G};
\item
Debugger JavaScript\ped{G};
\item
